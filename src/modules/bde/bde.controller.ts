...
import { BdeService, CreateBdeDto, UpdateBdeDto } from './bde.service';
...
export class BdeController {
   ...
   */
  async createBde(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      const body = request.body as CreateBdeDto;
      if ((body as any).universite_id) delete (body as any).universite_id;
   ...
   * Get BDE by ID
   */
  async getBdeById(request: FastifyRequest, reply: FastifyReply) {
    try {
   ...
   */
  async getBdeByUniversiteId(request: FastifyRequest, reply: FastifyReply) {
    try {
      const { universite_id } = request.params as { universite_id: string };
   ...
   */
  async updateBde(request: FastifyRequest, reply: FastifyReply) {
    try {
      const user = (request as any).user;
      if (!user) {
        return reply.status(401).send({ success: false, error: 'Unauthorized' });
      ...
      const { id } = request.params as { id: string };
      const body = request.body as UpdateBdeDto;
   ...
   */
  async deleteBde(request: FastifyRequest, reply: FastifyReply) {
    try {
      const user = (request as any).user;
      if (!user) {
        return reply.status(401).send({ success: false, error: 'Unauthorized' });
